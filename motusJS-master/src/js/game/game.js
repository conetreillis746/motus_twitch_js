...
function addScoreTeamFocus() {
    if (global.gamemode == "normal") {
        ...
        editHTML("score_0_panel", "innerHTML", '<div id="score_yellow"></div>' + game.score_yellow);
        editHTML("score_1_panel", "innerHTML", '<div id="score_blue"></div>' + game.score_blue);
        onEvent('editScore')
    }
}
...
function switchTeamFocus() {
    if (game.team_enabled == true) {
        $('.active_score').removeClass('active_score')
        if (game.team_focus == "yellow") {
            $("#score_1_panel").addClass("active_score");
            game.team_focus = "blue"
        } else {
            $("#score_0_panel").addClass("active_score");
            game.team_focus = "yellow"
        }
        console.log("L'équipe " + game.team_focus + " a maintenant la main.");
        onEvent('switchTeamFocus')
    } else {
...
function wordFoundInformation() {
    if (game.word_displayed == true || game.word_displayed == true) {
        return wordInformation(game.word_to_find);
    } else {
        errorHandler("Vous devez trouver le mot ou l'afficher pour pouvoir avoir des informations dessus.", false)
    }
}

function wordInformation(word_to_search) {
    return "https://fr.wiktionary.org/w/index.php?search=" + word_to_search.toLowerCase();
...
function errorHandler(errorCode, catching_up) { // Affiche dans la console le terme de l'erreur
    ...
    switch (errorCode) {
        case 1:
            texte = ("La longueur du mot à trouver n'est pas la bonne.");
        break;
        case 2:
            texte = ("Mot non présent dans le dictionnaire");
        break;
        case 3:
            texte = ("Mot déjà proposé");
        break;
        case 4:
            texte = ("Mot déjà affiché");
        break;
        case 5:
            texte = ("Boule non valide");
        break;
        case 6:
            texte = ("Boule non valide");
        break;
    ...
    console.log(texte)
    onEvent('errorHandler', texte)
    // TODO : Avoir le texte qui s'affiche pour les joueurs
    playsound("error");
